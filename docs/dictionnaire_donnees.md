...
- ⚠️ **Les `tch_` dépassent 100 % sur toutes les filières, et le solaire est le cas le plus bénin.** Un taux de charge rapporte une production à la puissance installée : il ne peut pas dépasser 100 %. Mesuré sur l'ensemble du jeu :

  | Variable | Lignes > 100 % | Part | Maximum | Régions les plus touchées |
  |---|---|---|---|---|
  | `tch_nucleaire` | 4 098 | 0,53 % | 102,01 % | Nouvelle-Aquitaine, Centre-Val de Loire |
  | `tch_eolien` | 696 | 0,05 % | 125,56 % | Île-de-France, Pays de la Loire |
  | `tch_hydraulique` | 89 | 0,007 % | **2 575 %** | Normandie, Hauts-de-France |
  | `tch_bioenergies` | 74 | 0,006 % | **718,75 %** | Île-de-France, Auvergne-Rhône-Alpes |
  | `tch_solaire` | 56 | 0,004 % | 172,49 % | Grand Est, Hauts-de-France, Bretagne |

  Lecture : la puissance installée de référence est **en retard sur le parc réel**. L'ampleur du dépassement suit la **petitesse du parc régional** et non la filière : l'hydraulique en Normandie ou les bioénergies en Île-de-France portent quelques mégawatts, où la moindre erreur de référence explose en pourcentage. Le nucléaire, au parc énorme, ne dépasse jamais 102 %.

  **Conséquence pratique** : `capacite_installee()` inverse précisément cette variable, ses valeurs sont donc **sous-estimées** aux mêmes endroits. Sans effet sur les analyses actuelles, qui emploient des médianes, mais bloquant pour toute lecture ponctuelle de la capacité, et **à éviter complètement** pour l'hydraulique et les bioénergies au niveau régional.
